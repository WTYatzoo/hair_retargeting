...
int io::getVertexAndQuadFromObj(std::vector<vertex> &myvertexs,std::vector<quad > &myquads,const std::string name)
{
  ...
  for (MyMesh::FaceIter f_it=mesh.faces_begin();f_it!=mesh.faces_end();++f_it)
	...
	{ 
      ...
      myquads.push_back(quad(index_vertex));            
      num_face++;
    }
  printf("num vertex: %d num_face: %d\n",now,num_face);  
  return 0;
...
int io::getVertexAndHairFromObj(std::vector<vertex > &myvertexs_hair,std::vector<e > &myedges_hair,const std::string name)
{
  char filter[50];  
  ...
  while(!feof(file))
      ...
      while(filter[0]=='v'&&filter[1]=='\0')
	{
	  ...
	  vertex vertex_now=vertex(myvector(a,b,c));
	  myvertexs_hair.push_back(vertex_now);
	  fscanf(file,"%s",filter);
	}
      while(filter[0]!='l'||filter[1]!='\0')
	{
	  fscanf(file,"%s",filter);
	}
      ...
      while(filter[0]=='l'&&filter[1]=='\0'&&!feof(file))
	{	  
	  e e_here;
	  fscanf(file,"%d %d",&e_here.v1,&e_here.v2);
	  e_here.v1--; e_here.v2--;
	  e_here.c0=c0; e_here.c1=c1; e_here.c2=c2;
...
int io::saveHairAsVTK(std::vector<vertex > &myvertexs_hair,std::vector<e > &myedges_hair,const std::string name)
{
  int i,j;
  FILE *fp; 
  fp=fopen(name.c_str(),"w");
  ...
  fprintf(fp,"DATASET UNSTRUCTURED_GRID\n");
  fprintf(fp,"POINTS %d double\n",myvertexs_hair.size());
  for(i=0;i<myvertexs_hair.size();++i)
    {
      fprintf(fp,"%lf %lf %lf\n",myvertexs_hair[i].location.x,myvertexs_hair[i].location.y,myvertexs_hair[i].location.z);
    }
  fprintf(fp,"CELLS %d %d\n",myedges_hair.size(),myedges_hair.size()*3);
  for(i=0;i<myedges_hair.size();++i)
    {
      fprintf(fp,"2 %d %d\n",myedges_hair[i].v1,myedges_hair[i].v2);
    }
  fprintf(fp,"CELL_TYPES %d\n",myedges_hair.size());
  for(i=0;i<myedges_hair.size();++i)
    {
  ...
  fprintf(fp,"CELL_DATA %d",myedges_hair.size());
  fprintf(fp,"SCALARS sample_cell double 1\n");
  fprintf(fp,"LOOKUP_TABLE rgbtable_cell\n");
  for(i=0;i<myedges_hair.size();++i)
    {
      fprintf(fp,"%f\n",float(i)/myedges_hair.size());
    }
  fprintf(fp,"LOOKUP_TABLE rgbtable_cell %d\n",myedges_hair.size());
  for(i=0;i<myedges_hair.size();i++)
    {
      fprintf(fp,"%f %f %f 1\n",myedges_hair[i].c0,myedges_hair[i].c1,myedges_hair[i].c2);
    }
